...
import modules
import modules_finer_plus_plus
...
def init_model(opt):
    ...
    if opt.model_type == 'mfn':
        ...
        model = modules.FinerB(
            ...
            first_bias_scale=opt.fbs,
            is_sdf=True
        )
        print(model)
    
    elif opt.model_type == 'nerfpe':
        model = modules.CoordinateNet(nl='relu',
                                      in_features=3,
                                      out_features=1,
                                      num_hidden_layers=opt.hidden_layers,
                                      hidden_features=opt.hidden_size,
                                      is_sdf=True,
                                      pe_scale=opt.pe_scale, nerfpe=True,
                                      use_sigmoid=False)

    elif opt.model_type == 'gauss':
        model = modules_finer_plus_plus.Gauss(
    ...
    elif opt.model_type == 'gf':
        model = modules_finer_plus_plus.GF(
            in_features=3, out_features=1, hidden_layers=opt.hidden_layers, hidden_features=opt.hidden_size, 
            scale=3, omega=10, 
            init_method='sine', init_gain=1, fbs=opt.fbs, is_sdf=True
        )
        print(model)
    elif opt.model_type == 'wf':
        model = modules_finer_plus_plus.WF(
            in_features=3, out_features=1, hidden_layers=opt.hidden_layers, hidden_features=opt.hidden_size,
            scale=2, omega_w=4, omega=5,
            init_method='sine', init_gain=1, fbs=opt.fbs, is_sdf=True
        )
        print(model)
    elif opt.model_type == 'SDFNetworkSIREN':
