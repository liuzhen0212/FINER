...
class SIRENX(hk.Module):
    ...
    def __call__(self, coords):
        sh = coords.shape
        x = jnp.reshape(coords, [-1, 2])
        x = SIRENXLayer(x.shape[-1], self.width, is_first=True, w0=self.w0, bs=self.bs)(x)
        x = jnp.sin(x)

        for _ in range(self.depth - 2):
            x = SIRENXLayer(x.shape[-1], self.width, w0=self.hidden_w0)(x)
            x = jnp.sin(x)

        out = SIRENXLayer(x.shape[-1], 1, w0=self.hidden_w0, is_last=True)(x)
        out = jnp.reshape(out, list(sh[:-1]) + [1])
